...
public readonly struct Angle : IEquatable<Angle>
{
	...
	public static Angle FromQuaternion(Quaternion quaternion, RotationAxis rotationAxis = RotationAxis.Y)
	{
		...
		if (test > 0.4999f) // Singularity at north pole
		{
			...
			{
				case RotationAxis.Y:
					return new Angle(2 * Mathf.Atan2(quaternion.x, quaternion.w));
				case RotationAxis.Z:
					return new Angle(PIRad / 2);
				case RotationAxis.X:
					return new Angle(0);
				default:
					throw new Exception("This cannot happened! Check input axis.");
			}
		}
		...
		{
			case RotationAxis.Y:
				return new Angle(Mathf.Atan2(2 * quaternion.y * quaternion.w - 2 * quaternion.x * quaternion.z,
					1 - 2 * (quaternion.y * quaternion.y) - 2 * (quaternion.z * quaternion.z)));
			case RotationAxis.Z:
				return new Angle(Mathf.Asin(2 * test));
			case RotationAxis.X:
				return new Angle(Mathf.Atan2(2 * quaternion.x * quaternion.w - 2 * quaternion.y * quaternion.z,
